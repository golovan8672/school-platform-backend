...
router.get("/me", auth, async (req, res) => {
    const user = await (await Student.findById(req.user.userId).select("-password").select("-__v")).select("-_id") || await Teacher.findById(req.user.userId).select("-password").select("-v") || await Moderator.findById(req.user.userId).select("-password").select("-v");
    res.send(user);
  });
